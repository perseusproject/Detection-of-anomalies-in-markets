...
from old_backtester import strategy, calculate_pnl


# Define a list of ticker symbols
tickers = ["FEZ", "EWU", "DAX", "EWQ", "IEUR"]
...
if not returns_matrix_Xt.empty:
    ...
    for j, ticker in enumerate(tickers):
        ...
        signals = strategy(returns_matrix_Xt, sparse_component, j)

        # Get the returns series for the current asset
        returns_series = returns_matrix_Xt.iloc[j]

        # Calculate PnL for the current asset
        pnl = calculate_pnl(returns_series, signals)
        all_pnl[ticker] = pnl
        print(f"Total PnL for {ticker}: {pnl.iloc[-1]:.2f}")

    print("\n--- Backtesting Results ---")
    # Plot PnL for all assets
    plt.figure(figsize=(12, 6))
    all_pnl.plot(ax=plt.gca())
    plt.title('Cumulative PnL of the Strategy for Each Asset')
    plt.xlabel('Time')
    ...
    # Define filename for PnL chart
    pnl_filename = os.path.join("Figures", f"{period}{interval}PnL.png")

    # Check if the PnL chart file already exists before saving
    if not os.path.exists(pnl_filename):
        plt.savefig(pnl_filename)
        print(f"PnL chart saved to {pnl_filename}")
    else:
        print(
            f"PnL chart file already exists at {pnl_filename}, skipping save.")
